...
// ─ ADMIN TEAM CRUD ─
async function createAdminMember(member_name, member_role, member_department, member_photo_url = '', member_email = '', display_order = 0) {
  const { data, error } = await supabaseDb.from('about_admin_team').insert([{
    member_name, member_role, member_department, member_photo_url, member_email, display_order, is_active: true
  }]).select();
