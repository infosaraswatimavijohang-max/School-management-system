-- ════════════════════════════════════════════════════════════════════════════
-- SCHOOL DOCUMENTS MANAGEMENT SYSTEM - DATABASE SETUP (FIXED)
-- ════════════════════════════════════════════════════════════════════════════
-- This SQL script creates/fixes the necessary tables for documents
...
ALTER TABLE public.school_documents
  ADD COLUMN IF NOT EXISTS file_name TEXT NOT NULL DEFAULT 'document',
...
DROP POLICY IF EXISTS "Allow authenticated users to read active documents" ON public.school_documents;
DROP POLICY IF EXISTS "Allow authenticated users to insert documents" ON public.school_documents;
DROP POLICY IF EXISTS "Allow authenticated users to update documents" ON public.school_documents;
DROP POLICY IF EXISTS "Allow authenticated users to delete documents" ON public.school_documents;

-- Allow anyone to read active documents
CREATE POLICY "Allow public to read active documents"
ON public.school_documents
FOR SELECT
USING (is_active = TRUE);

-- Allow authenticated admins to manage documents
